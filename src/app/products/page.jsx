...
export default function AllProductsPage() {
  ...
  // Initialize filters based on products
  useEffect(() => {
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
        ...
        <div className="flex flex-col lg:flex-row gap-8">
          ...
          <div className={`lg:w-80 ${showFilters ? 'fixed inset-y-0 left-0 z-50 w-80 bg-white shadow-xl lg:relative lg:shadow-sm' : 'hidden lg:block'}`}>
            <div className="bg-white rounded-lg shadow-sm p-6 sticky top-8 h-full lg:h-auto overflow-y-auto">
                    ...
                    <div className="space-y-2">
                      {filterOptions[filterType].map((option) => (
                        <label key={option} className="flex items-center gap-2 text-sm text-gray-700">
                          <input
                            type="checkbox"
                            checked={selectedFilters[filterType]?.includes(option?.toString()) || false}
                            onChange={() => toggleFilter(filterType, option?.toString())}
